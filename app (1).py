...
# ── Session state init ────────────────────────────────────────────────────────
def show_pipeline_error(error: Exception):
    message = str(error)
    lower_message = message.lower()

    if (
        "permission-denied" in lower_message
        or "doesn't have any credits or licenses" in lower_message
        or "error code: 403" in lower_message
    ):
        st.error(
            "The model provider rejected the request because the account does not have credits "
            "or an active license yet."
        )
        st.info(
            "Check the account linked to your API key, then restart Streamlit "
            "and run again."
        )
    else:
...
if run_btn:
    ...
    if not topic.strip():
        ...
        key for key in ("GROQ_API_KEY", "TAVILY_API_KEY")
        if not os.getenv(key)
    ]:
        st.error(
            "Missing environment keys: "
            + ", ".join(missing_keys)
            + ". Create a .env file in the project folder and restart Streamlit."
        )
        st.code(
